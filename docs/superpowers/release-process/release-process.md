...
- Catalog `INDEX.md` totals correct? Manually count the catalog rows (or run a `wc -l` against the catalog source) and compare against the totals reported in `power-engineer/references/catalog/INDEX.md`. Any mismatch is drift.
- `.catalog-version` matches the CHANGELOG `### Catalog` block declared version? Read both files and confirm the version strings agree. A mismatch means either the catalog was bumped without a CHANGELOG entry or the CHANGELOG promised a bump that the `.catalog-version` file never received.
- Any uncommitted changes to non-shipping files? Run `git status` and confirm the working tree is clean. "Non-shipping files" means anything outside `power-engineer/` plus the maintainer-only surfaces (`docs/`, `scripts/`, `tests/`, `.github/`) — uncommitted edits to these files at release time indicate either incomplete work or accidentally-staged scratch state.
...
**Fix protocol:** drift fixes happen in a SEPARATE commit, NOT amended into a release commit. Use commit-message format `chore: drift-sweep <Nx items>` followed by a bullet list of the specific drifts corrected (one bullet per finding). For example: `chore: drift-sweep 3x items` with bullets for the README badge resync, the alt-text correction, and the `.catalog-version` bump. The separate-commit rule preserves the audit trail: a future archaeologist reading `git log` can distinguish between intentional release content and the drift-sweep delta. This protocol is symmetric to the in-place template fix protocol described in §9 (Template Dogfooding Expectation): both philosophies accept that a fix lands in the same release as the formalization or template that surfaced it, but each requires the fix to be commit-isolated so the provenance is legible.

**Audit:** Sonnet by default. Drift-sweep is a mechanical check — re-run each command, diff the output against the implementer's reported state, confirm no items were skipped. Upgrade to Opus only when a finding requires judgment (e.g., an ambiguous "what does correct alt-text mean for this screenshot" question that the implementer flagged for escalation). Most drift-sweep audits are unambiguous and Sonnet is the right tier; the Opus upgrade should be the exception, not the rule.
