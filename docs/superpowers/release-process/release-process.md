...
## 11. Drift Sweep (Phase 8.5)

Drift-sweep is a mandatory release step. It exists because the two prior releases each surfaced drift through luck rather than process: v1.4.0's release-day audit caught alt-text drift on README badges only because the final auditor happened to look at the rendered images, and v1.4.1's dogfood pass surfaced an awk-range defect in a release-process template that had been silently broken since introduction. Both findings were serendipitous — neither was the result of a checklist item that named "drift" and demanded evidence. The cost of NOT formalizing this step is that every future release re-discovers the same drift class through luck: the next maintainer (or the next orchestrator subagent) has no signal that drift-sweep is part of the release contract, so they skip it, and a stale badge or bad alt-text or out-of-sync version metadata ships unflagged. Formalizing the step removes the dependency on auditor improvisation and makes drift detection a graded gate.

**What to check** (run each item, capture output, attach to the phase verification report):

- README badges current? Run `scripts/update-skill-count.sh` and verify `git status` reports no diff in the working tree afterward. A non-empty diff means the README was stale; the script has now corrected it and the correction must ship as a drift-sweep commit.
- Catalog `INDEX.md` totals match the script output? `scripts/update-skill-count.sh` re-counts catalog table rows across all catalog files (excluding `INDEX.md`) and writes the result into BOTH the README badge AND `INDEX.md`'s `*Total skills catalogued: N*` line. After running the script, if `git status` shows a diff to either file, the catalog row count drifted from what was previously committed — the README/INDEX claim was stale and the script just corrected it. Ship the corrected files as a `chore: drift-sweep` commit (per the Fix protocol below).
- `power-engineer/.catalog-version` matches the `CHANGELOG.md`'s `### Catalog` block declared version? Read both files and confirm the version strings agree. A mismatch means either the catalog was bumped without a CHANGELOG entry or the CHANGELOG promised a bump that the `power-engineer/.catalog-version` file never received.
- Any uncommitted changes to non-shipping files? Run `git status` and confirm the working tree is clean. "Non-shipping files" means anything outside `power-engineer/` — i.e., the maintainer-only surfaces such as `docs/`, `scripts/`, `tests/`, `.github/` — uncommitted edits to these files at release time indicate either incomplete work or accidentally-staged scratch state.
- Alt-text on README images matches the rendered image content? For each `![alt text](image-url)` in the README, confirm the alt text accurately describes what the image renders (badge counts, screenshot contents, diagram labels). Stale alt text is the v1.4.0 drift class — a count or label that was correct when the image was first added but is no longer accurate after a catalog or feature change.

**Fix protocol:** drift fixes happen in a SEPARATE commit, NOT amended into a release commit. Use commit-message format `chore: drift-sweep <Nx items>` followed by a bullet list of the specific drifts corrected (one bullet per finding). For example:

```
chore: drift-sweep 3x items

- README badge resync (228 → 230)
- README alt text "228 Skills" → "230 Skills"
- power-engineer/.catalog-version 1.4.0 → 1.4.1
```

The separate-commit rule preserves the audit trail: a future archaeologist reading `git log` can distinguish between intentional release content and the drift-sweep delta. This protocol is symmetric to the in-place template fix protocol described in §9 (Template Dogfooding Expectation): both philosophies accept that a fix lands in the same release as the formalization or template that surfaced it, but each requires the fix to be commit-isolated so the provenance is legible.

**Audit:** Sonnet by default. Drift-sweep is a mechanical check — re-run each command, compare each command's output against the implementer's reported finding, confirm no items were skipped. Upgrade to Opus only when a finding requires judgment (e.g., an ambiguous "what does correct alt-text mean for this screenshot" question that the implementer flagged for escalation). Most drift-sweep audits are unambiguous and Sonnet is the right tier; the Opus upgrade should be the exception, not the rule.

**Insertion point in the release plan:** for releases that follow the executor-template's standard 9-phase shape (v1.3.0, v1.4.0), drift-sweep is Phase 8.5 — it lands BETWEEN Phase 8 (Docs + Integration) and Phase 9 (Release Ceremony). The half-integer phase number is intentional: it signals that drift-sweep is a checkpoint gate rather than a content phase. For releases that use the lightweight 4-phase shape (v1.4.1, v1.4.2 and other QoL releases that compress implementation + tests + docs into fewer phases), drift-sweep happens as the FIRST task of Phase 3 (Release Ceremony) — its position immediately before CHANGELOG/MIGRATION/version-bump work preserves the same "drift detected before tag" guarantee that Phase 8.5 provides in the standard shape.
