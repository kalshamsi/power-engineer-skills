...
- Catalog `INDEX.md` totals match the script output? `scripts/update-skill-count.sh` writes both the README badge AND `INDEX.md`'s `*Total skills catalogued: N*` line; if either disagrees with `find power-engineer/references/catalog -name '*.md' ! -name 'INDEX.md' | wc -l`'s row count, the script's awk pattern itself has drifted (a deeper class of defect).
